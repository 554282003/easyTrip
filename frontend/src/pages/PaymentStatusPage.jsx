...
function PaymentStatus() {
  ...
  const checkServerStatus = async () => {
    try {
      const response = await fetch("http://localhost:5000/ha", {
        method: "GET",
        headers: {
  ...
  useEffect(() => {
    ...
    checkServerStatus().then((isServerRunning) => {
      ...
      // Verify payment if order ID exists
      if (orderId) {
        verifyPayment()
  ...
  const verifyPayment = async () => {
    try {
      ...
      if (data.status === "PAID") {
          ...
          if (storedPackageId) {
            ...
            if (bookingDataParam && packageDataParam) {
              try {
                ...
                retrievedBookingDetails = parsedBookingDetails
                retrievedPackageDetails = parsedPackageDetails
        ...
        // If we have booking details after the retrieval attempt, send receipt email automatically
        // Use a longer timeout to ensure state updates have completed
        setTimeout(() => {
          console.log("Preparing to send email with:", {
            hasBookingDetails: !!retrievedBookingDetails,
            hasPackageDetails: !!retrievedPackageDetails,
          })
          ...
          if (retrievedBookingDetails && retrievedPackageDetails) {
            ...
            // Send the email
            sendReceiptEmail(updatedOrderDetails, retrievedBookingDetails, retrievedPackageDetails).then(() => {
            ...
            console.error("Cannot send receipt email: missing booking or package details")
            setToastMessage("Could not send receipt email: missing booking details")
            setToastType("error")
            setShowToast(true)
            setIsEmailSendingComplete(true) // Set as complete even if there's an error
          }
  ...
  const downloadReceipt = async () => {
    try {
      ...
      link.href = url
      link.download = `FlyAnyTrip_Receipt_${paymentStatus.orderDetails.order_id}.pdf`
